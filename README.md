...
| `max_tokens` | `int` | `None` | 可选的累计 **token** 上限——与 USD 上限并行，先触顶者为准。 |
...
- [x] **v0.2 · 加固**：未定价模型失败即熔断（`on_unpriced`）、token 上限（`max_tokens`）与单次调用硬上限（`single_call_ceiling`），外加一个可选的 JSONL 花费记录，喂给 `agentfuse status --log`。
