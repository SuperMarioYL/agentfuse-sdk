...
`agentfuse demo` 一键复现 lantian 那条帖子里的失控循环，并当场把它掐断——**完全离线**（litellm `mock_response`，无需 API key）：

```bash
agentfuse demo --ceiling 0.50
```

<!-- demo.gif 由 assets/demo.tape 通过 `vhs assets/demo.tape` 生成（见 assets/README.md） -->
<p align="center">
  <img src="assets/demo.gif" alt="AgentFuse 在失控 Agent 触顶前跳闸的终端录屏" width="760" />
</p>

> 📼 GIF 暂未提交——运行 `vhs assets/demo.tape` 即可生成（见 [assets/README.md](./assets/README.md)）。下面是这条命令的真实输出：
...
> v0.1 没有持久化花费存储（明确不在范围内）——台账只存在于一次运行的 `with Fuse(...)` 作用域里，不跨进程、不跨 run。`agentfuse status` 会如实说明这一点。
...
<sub>本仓库由 <a href="https://github.com/supermario_leo">ai-radar</a> 信号扫描所驱动的产品方案孵化而来。</sub>
