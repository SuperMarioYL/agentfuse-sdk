"""The bundled offline runaway-agent demo, vendored inside the package.

This is the demo ``agentfuse demo`` runs. It reproduces, in miniature, the
failure mode from the viral "AI agent bankrupted its operator while scanning
DN42" thread: an autonomous agent stuck in a loop, firing paid LLM call after
paid LLM call with nobody watching the meter.
...
This module lives inside the installed package (``src/agentfuse``) so that
``agentfuse demo`` works from a ``pip install`` of the wheel — ``examples/`` is
NOT shipped in the wheel (only ``src/agentfuse`` is packaged), so the demo could
not previously be found from a wheel install. The repo-root
``examples/runaway_agent.py`` is now a thin re-import of this module so
``python examples/runaway_agent.py`` from a source checkout still works.
"""
...
def run_demo(ceiling_usd: float = 0.50, max_iterations: int = 100) -> None:
    ...
    snap = budget.snapshot()
    print(f"\nTask finished — spent ${snap.spent_usd:0.4f} / ${snap.ceiling_usd:0.2f}.")
